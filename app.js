...
(function () {
  ...
  function setTheme(theme) {
    ...
    storage.set('tisser-theme', theme);
    if (themeToggle) {
  ...
  const savedTheme = storage.get('tisser-theme', null);
  if (savedTheme) {
    setTheme(savedTheme);
  ...
  function setPreviewMode(mode) {
    ...
    storage.set('tisser-preview-mode', safeMode);
  }

  const savedPreview = storage.get('tisser-preview-mode', 'auto');
  ...
  const savedChecklist = storage.get('tisser-checklist', []);
  ...
  checklistItems.forEach((item) => {
    item.addEventListener('change', () => {
      storage.set('tisser-checklist', checklistItems.map((checkbox) => checkbox.checked));
    });
  });
